...
const useKeyDetector = ({ key, delayBy = 0, action }) => {
	...
	function onKeyDown(e) {
		keys.forEach((key) => {
			...
			const keyName = keyCodeMap[e.keyCode]?.toLowerCase();

			if (["Meta", "Shift", "Control", "Option", "Alt"].includes(e.key))
				return;

			if (
				(key.includes("Shift") && e.shiftKey) ||
				(key.includes("Cmd") && (e.metaKey || e.ctrlKey)) ||
				((key.includes("Option") || key.includes("Alt")) && e.altKey) ||
				(key.includes("Ctrl") && e.ctrlKey)
			) {
				if (validKey != keyName) return;
