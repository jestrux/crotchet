...
import { ActionRecord } from './registry';
import { oauth } from './globals/auth';
...
export const builtinActions: ActionRecord[] = [
  {
    name: 'spotifyPlayer',
    label: 'Spotify Player',
    icon: { type: 'svg', path: spotifyIconPath, filled: true },
    color: '#03BD7D',
    handler: async () => {
      ...
      useSpotifyPlayerStore.getState().open(token);
    },
  },
];
